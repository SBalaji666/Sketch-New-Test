...
export function generateCutList(config) {
  ...
  const { carcass: ct, back: bt, shelf: sht, door: dt, drawerSide: dst, drawerBottom: dbt } = materials;
  const { doorGap: dg, edgeBanding: ebt, sawKerf: sk } = tolerances;
  
  const joinery = JOINERY_TYPES[hardware.joinery.toUpperCase()] || JOINERY_TYPES.BUTT;
  ...
  const doorOverlay = DOOR_OVERLAY_TYPES[hardware.doorOverlay] || DOOR_OVERLAY_TYPES.FULL;
  const construction = CONSTRUCTION_TYPES[hardware.construction] || CONSTRUCTION_TYPES.ASSEMBLED;
  ...
  add({
    ...
    edgeBand: 'front edge',
    note: 'Full width top',
    machining: joinery.requiresBoring ? 'Dowel/cam holes required on underside' : 'None',
  });
  ...
  add({
    ...
    machining: joinery.requiresBoring ? 'Dowel/cam holes required on topside' : 'None',
  });
  ...
  add({
    ...
    machining: joinery.id === 'dado' ? 'Dado grooves for dividers' : joinery.requiresBoring ? 'Boring for joinery' : 'None',
  });
  ...
  if (dividerCount > 0) {
    ...
    add({
      ...
      machining: joinery.requiresBoring ? `Boring for ${joinery.name}` : joinery.id === 'dado' ? 'Sits in dado groove' : 'Butt joint',
    });
  }
  ...
  const normSections = sections.map(s => ({ ...s, _actualWidth: Math.round((s.width / totalW) * L) }));
  ...
  normSections.forEach((section, idx) => {
    ...
    const interiorWidth = sectionWidth - ct * (isFirstSection || isLastSection ? 1 : 2);
    ...
    if (section.shelves > 0) {
      ...
      add({
        ...
        machining: shelfSys.id === 'fixed' ? 'Dado into sides or screwed' : 'None',
        hardware: shelfSys.adjustable ? `${shelfSys.name} × ${shelfSys.pinsPerShelf} per shelf` : 'None',
      });
    ...
    if (drawerCount > 0) {
      const slide = DRAWER_SLIDES[hardware.drawerSlide] || DRAWER_SLIDES['blum-tandem-550'];
      const drawerHeight = section.drawers.height;
      
      // Check if drawer height is compatible with selected slide
      const slideCompatible = slide.heights[0] <= drawerHeight && drawerHeight <= slide.heights[slide.heights.length - 1];
      
      const drawerBoxWidth = interiorWidth - slide.clearancePerSide * 2 - ebt * 2;
      ...
      add({
        ...
        machining: construction.requiresCamLocks ? 'Cam lock boring' : 'Groove for bottom panel',
      });
...
export function generateHardwareSchedule(config) {
  ...
  // Count doors
  const doorCount = sections.filter(s => s.type === 'closed' && s.drawers.count === 0).length;
...
export function generateMachiningSchedule(cutList, config) {
  ...
  if (joinery.requiresBoring) {
    const panels = cutList.filter(p => p.machining && p.machining.includes('Boring'));
    panels.forEach(panel => {
      machining.push({
        part: panel.part,
        section: panel.section,
        operation: joinery.id === 'dowel' ? 'Dowel Boring' : 'Cam Lock Boring',
        tool: joinery.id === 'dowel' ? `${joinery.dowelSize}mm drill` : `${joinery.camSize}mm Forstner`,
